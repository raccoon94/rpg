...
function heroGear(h){ return (typeof equippedGearForHero==='function' ? equippedGearForHero(h) : null); }
function heroGearStats(h){ return (typeof gearStats==='function' ? gearStats(heroGear(h)) : {atk:0,hp:0,def:0,spd:0,crit:0,critDmg:0,heal:0}); }
function heroEquipSum(h){ return 0; }
...
function heroDef(h){
  ...
  return Math.round(((28 + heroLevel(h)*5 + heroAscCount(h)*55) * mult * role + (heroGearStats(h).def||0)) * petDefMult());
}
...
function gearAtkTotal(){ return partyHeroes().reduce((s,h)=>s+(heroGearStats(h).atk||0),0); }
